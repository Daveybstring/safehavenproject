...
For each input parameter on the form I made sure to unclude protection to protect against sql injection.<br>
...
When a user trys to login the form will check the users information against the database.<br>
</p>
</details>
...
<br>
 <details>
  ...
  <p> All form inputs are protected against sql injection.<br> They use mysql real escape string to escape the single quotations which can be 
  placed around statements which if entered into an input can be made to look like they were part of the original script.<br> 
  The script may by accident execute these and return sensitive information from the database.<br> </p>
</details>
<br>
 <details>
  <summary><h3><span title="Click here for more info">Hash and salt passwords</span></h3></summary>
  <p>When a user registers and enters in what they wish there password to be I will hash and salt the password. <br> 
  For every user the salt is unique so the with the salt being unique to every user the salt will never be known.<br> 
  We take the password and use it as a salt, we md5 the password string which is now a salt. <br> 
  We then sha1 this entire string and add it to the original password and sha1 both of them together combining the password and the salt.<br></p>
</details>
<br>
 <details>
  <summary><h3><span title="Click here for more info">Protect index directory</span></h3></summary>
  <p>Originally the index directory wasn't protected so you could easily access the .php files of the website. <br> 
  Dangerous considering if a person downloaded these they would be able to edit them and perhaps find out the key that we are using for encyrption.<br></p>
</details>
<br>
 <details>
  <summary><h3><span title="Click here for more info">Encrypt data within the database</span></h3></summary>
  <p>By using 2 functions which encrypt and decrypt data with the use of special key to encrypt and decrypt.<br> 
  It encrypts all data sent to the database and stores it there encrypted. When I want to display a user's profile 
  I simply echo it out using the decrypt function and it decrypts everything. <br>The key is hashed using sha1 so can't be unhashed.<br> 
  If a hacker breaks into my database they will never be able to decrypt the data without the key.<br> </p>
</details>
